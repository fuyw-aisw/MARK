...
def clustering(feature, n_clusters, kmeans_device='cpu',device=torch.device('cuda:0'), seed=42, batch_size=100000, tol=1e-4, spectral_clustering=False):
    if spectral_clustering:
        ...
        if kmeans_device == 'cuda':
            ...
            predict_labels, _ = kmeans(
                X=feature, num_clusters=n_clusters, batch_size=batch_size, tol=tol, device=device,seed=seed)
